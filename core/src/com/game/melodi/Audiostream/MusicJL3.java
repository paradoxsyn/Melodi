...
public class MusicJL3 extends AbsMusic implements AudioDeviceListener{
	...
	public float getTotalLength() {
		/*try {
			File f = new File("C:\\Users\\Paradox\\Desktop\\Melodi\\android\\assets\\furelise.mp3");
			AudioFile afile = AudioFileIO.read(f);
			int length;
			System.out.println(afile.getAudioHeader().getTrackLength());
			length = afile.getAudioHeader().getTrackLength();
		...
		int medLen = (int) file.file().length();
		System.out.println("File Length?"+medLen);
		*/
		float tn = 0;
		try{
